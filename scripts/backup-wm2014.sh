...
wget --save-cookies $cookies --keep-session-cookies --load-cookies $cookies "${site}" -O $TMPDIR/login_form
...
wget --save-cookies $cookies --keep-session-cookies --load-cookies $cookies \
     --post-data="form.submitted&_csrf=$csrf&alias=$name&password=$pass" "http://${site}/login" -O /dev/null

wget --keep-session-cookies --save-cookies $cookies --load-cookies $cookies "http://${site}/update_local" -O update_local.html
